...
#         return np.multiply(x_j.detach().numpy(), weights)
        # so not actually weighted :/
        return x_j
...
def gaussian_kernel(d_ij):
    d_ij_detached = d_ij.detach().numpy()
    return np.exp(-d_ij_detached**2)
...
#         print("features: ", x)
#         print("mean of features: ", torch.mean(x))
        x = torch.cat([x, torch.mean(x,1).unsqueeze(-1)], dim=1)
...
class GravNet(nn.Module):
    ...
    def __init__(self, n_blocks = 4, final_dim = 128, n_clusters = 2, **kwargs):
        ...
        original_input_dim = kwargs['input_dim']
        self.inputnorm = nn.BatchNorm1d(original_input_dim)
        ...
        kwargs['input_dim'] = original_input_dim
